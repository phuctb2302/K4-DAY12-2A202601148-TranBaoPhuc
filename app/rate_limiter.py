...
class TokenBucket:
    ...
    def available(self, client_id: str, now: float | None = None) -> float:
        ...
        # Handle both bytes (from pure redis) and strings (if decode_responses=True)
        tokens_val = state.get(b"tokens") or state.get("tokens") or 0
        last_val = state.get(b"ts") or state.get("ts") or 0
